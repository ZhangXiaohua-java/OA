...
import com.ruoyi.web.component.PoiComponent;
...
public class DispatchOrderMessageService {
	...
	public void dispatchOrder(Message message) {
		...
		orderTask.setPoiCode(orderVO.getPoiCode());
	...
	public void consumerDispatchMessage(Message message) {
		...
		List<PostOrderTo> orders = objects.stream().map(e -> {
		...
		List<DispatchOrderInfo> dispatchOrderInfos = orders.stream().map(e -> {
			...
			String poiCode = e.getPoiCode();
			dispatchOrderInfo.setPoiCode(poiCode);
			String dest = e.getDest();
			// 根据目的地分析出最近的营业厅
			//TODO 根据poi检索出最近的营业厅编号
			Pois poi = null;
			try {
				poi = poiComponent.searchLocationInfoByPoiCode(poiCode);
			...
			String bsRes = feignRemoteClient.queryMostNearlyBsInfo(position[0], position[1]);
			Result r = JSON.parseObject(bsRes, Result.class);
			if (r.getCode() != 200) {
				log.info("调用远程服务查询营业厅信息失败了");
				return null;
			}
			BusinessDepartment bs = JSON.parseObject(r.getData().get("data").toString(), BusinessDepartment.class);
			log.info("查询到的bs信息{}", bs);
			String unifiedCode = bs.getUnifiedCode();
			...
			dispatchOrderInfo.setUnifiedCode(unifiedCode);
